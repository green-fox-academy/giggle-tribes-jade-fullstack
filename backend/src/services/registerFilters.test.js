...
test('missing password', async () => {
  const result = await filterInput({username:"username"});
  expect(result).toEqual('Password is required.');
});

test('missing username', async () => {
  const result = await filterInput({password:"password"});
  expect(result).toEqual('Username is required.');
});

test('too short password', async () => {
  const result = await filterInput({username:"username",password:"passt"});
  expect(result).toEqual('Password must be 8 characters.');
});

test('valid input', async () => {
  const result = await filterInput({username:"username",password:"password"});
