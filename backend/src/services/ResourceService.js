export class ResourceService {
  ...
  async getByKingdomId({ kingdomId }) {
    ...
    if (resources.length === 0)
      throw new Error(this.errorCodes.invalidKingdomId);
  ...
  async add({ kingdomId, startAmount = 500 }) {
    if (!kingdomId) throw new Error(this.errorCodes.missingKingdomId);
    let resources = await this.resources.getByKingdomId({ kingdomId });
  ...
  async generateResources({ kingdomId }) {
    ...
    resources.forEach(async resource => {
      const minutes = Math.floor(
        (currentTime - new Date(resource.updatedAt)) / 60000
      );
  ...
  async resourceUpdateFactory(kingdomId, resourceType) {
    if (!kingdomId) throw new Error(this.errorCodes.missingKingdomId);
    const resources = (await this.resources.getByKingdomId({ kingdomId })).find(
      e => e.type === resourceType
    );
    const { amount, generation } = resources;
    return async (changeAmount, changeGeneration) => {
      await this.resources.update({
        amount: amount + changeAmount,
        generation: generation + changeGeneration,
        kingdomId,
        type: resourceType,
      });
    };
